...
class ResearchTrainingManager {
    ...
    async loadUserData() {
        try {
            if (this.userManager && this.userManager.hasUserId()) {
                ...
                this.researchData = userData?.scienceResearchData || this.getDefaultResearchData();
    ...
    loadFromLocalStorage() {
        ...
        this.researchData = JSON.parse(localStorage.getItem("scienceResearchData")) || this.getDefaultResearchData();
        return savedData;
    }
        ...
        try {
            ...
            if (this.userManager && this.userManager.hasUserId()) {
                ...
                await this.userManager.updateUserData({
                    scienceResearchData: this.researchData
                });
                ...
                localStorage.setItem("scienceResearchData", JSON.stringify(this.researchData));
        ...
        window.addEventListener('beforeunload', () => {
            ...
            if (this.isQuizActive && !this.researchData.quizResults) {
                try {
                    ...
                    if (this.userManager && this.userManager.hasUserId()) {
                        this.userManager.updateUserData({ scienceResearchData: this.researchData });
    ...
    async loadDailyTopic() {
        ...
        try {
            ...
            const prompt = `Generate a daily scientific learning topic from this category: ${randomCategory}
...
Please provide a response in this exact JSON format:
{
  ...
  "title": "Topic Title",
  "description": "A brief but engaging description of the scientific topic that would interest someone learning about it",
  "difficulty": "${difficultyRank}",
  "keyPoints": [
    "Key scientific point 1 - essential scientific concept to understand",
    "Key scientific point 2 - important scientific fact or principle",
    "Key scientific point 3 - core scientific idea or theory",
    "Key scientific point 4 - scientific context or practical application",
    "Key scientific point 5 - connection to broader scientific context"
  ]
}

CRITICAL REQUIREMENTS:
- difficulty MUST be exactly "${difficultyRank}"
- keyPoints MUST contain exactly 5 focused scientific learning objectives
- Each key point should be specific and scientifically accurate
- Make the topic interesting and educational about science
- Keep the description concise but informative
- Focus on essential scientific knowledge, not overwhelming details`;
 ...
 Make questions that test understanding of the topic while being engaging and educational.`;
    ...
    async selectAnswer(answer) {
        ...
        try {
            ...
            this.researchData.partialIndex = this.currentQuestionIndex;
            if (this.userManager && this.userManager.hasUserId()) {
                await this.userManager.updateUserData({ scienceResearchData: this.researchData });
                await this.userManager.forceSaveUserData();
            } else {
                localStorage.setItem("scienceResearchData", JSON.stringify(this.researchData));
            }

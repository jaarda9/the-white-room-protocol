...
class ResearchTrainingManager {
    ...
    async loadUserData() {
        try {
            if (this.userManager && this.userManager.hasUserId()) {
                ...
                console.log('🔍 Existing research data:', userData?.historyResearchData);
                
                this.researchData = userData?.historyResearchData || this.getDefaultResearchData();
    ...
    loadFromLocalStorage() {
        ...
        this.researchData = JSON.parse(localStorage.getItem("historyResearchData")) || this.getDefaultResearchData();
        return savedData;
    }
        ...
        try {
            ...
            if (this.userManager && this.userManager.hasUserId()) {
                ...
                await this.userManager.updateUserData({
                    historyResearchData: this.researchData
                });
                ...
                localStorage.setItem("historyResearchData", JSON.stringify(this.researchData));
        ...
        window.addEventListener('beforeunload', () => {
            ...
            if (this.isQuizActive && !this.researchData.quizResults) {
                try {
                    ...
                    if (this.userManager && this.userManager.hasUserId()) {
                        this.userManager.updateUserData({ historyResearchData: this.researchData });
    ...
    async loadDailyTopic() {
        ...
        try {
            // Call Gemini API with retries for robustness
            ...
            const categories = [
                ...
                'Medieval History (Feudalism, Crusades, Renaissance, Black Death)',
                'Modern History (World Wars, Industrial Revolution, Cold War)',
                'Historical Figures (Leaders, Inventors, Philosophers, Artists)',
                'Historical Events (Revolutions, Discoveries, Treaties, Conflicts)'
            ];
            ...
            const prompt = `Generate a daily historical learning topic from this category: ${randomCategory}
...
Please provide a response in this exact JSON format:
{
  ...
  "title": "Topic Title",
  "description": "A brief but engaging description of the historical topic that would interest someone learning about it",
  "difficulty": "${difficultyRank}",
  "keyPoints": [
    "Key historical point 1 - essential historical concept to understand",
    "Key historical point 2 - important historical fact or event",
    "Key historical point 3 - core historical idea or significance",
    "Key historical point 4 - historical context or impact",
    "Key historical point 5 - connection to broader historical context"
  ]
}

CRITICAL REQUIREMENTS:
- difficulty MUST be exactly "${difficultyRank}"
- keyPoints MUST contain exactly 5 focused historical learning objectives
- Each key point should be specific and historically accurate
- Make the topic interesting and educational about history
- Keep the description concise but informative
- Focus on essential historical knowledge, not overwhelming details`;
            ...
            while (attempt < maxAttempts) {
                try {
                    topicData = await doRequest(attempt + 1);
                    break;
                } catch (err) {
                    attempt++;
                    const isLast = attempt >= maxAttempts;
                    console.warn(`Topic generation attempt ${attempt} failed:`, err.message || err);
                    if (isLast) throw err;
            ...
            // Graceful fallback: if we have any previous topic today-like, show it
            if (this.researchData.currentTopic) {
                console.warn('Falling back to last saved topic');
                this.displayTopic(this.researchData.currentTopic);
                return;
            }
            const topicContent = document.getElementById('topic-content');
 ...
 Make questions that test understanding of the topic while being engaging and educational.`;
    ...
    async selectAnswer(answer) {
        ...
        try {
            ...
            this.researchData.partialIndex = this.currentQuestionIndex;
            if (this.userManager && this.userManager.hasUserId()) {
                await this.userManager.updateUserData({ historyResearchData: this.researchData });
                await this.userManager.forceSaveUserData();
            } else {
                localStorage.setItem("historyResearchData", JSON.stringify(this.researchData));
            }

...
class ResearchTrainingManager {
    ...
    async loadUserData() {
        try {
            if (this.userManager && this.userManager.hasUserId()) {
                ...
                this.researchData = userData?.geographyResearchData || this.getDefaultResearchData();
    ...
    loadFromLocalStorage() {
        ...
        this.researchData = JSON.parse(localStorage.getItem("geographyResearchData")) || this.getDefaultResearchData();
        return savedData;
    }
        ...
        try {
            ...
            if (this.userManager && this.userManager.hasUserId()) {
                ...
                await this.userManager.updateUserData({
                    geographyResearchData: this.researchData
                });
                ...
                localStorage.setItem("geographyResearchData", JSON.stringify(this.researchData));
        ...
        window.addEventListener('beforeunload', () => {
            ...
            if (this.isQuizActive && !this.researchData.quizResults) {
                try {
                    ...
                    if (this.userManager && this.userManager.hasUserId()) {
                        this.userManager.updateUserData({ geographyResearchData: this.researchData });
    ...
    async loadDailyTopic() {
        ...
        try {
            ...
            const categories = [
                'Physical Geography (Landforms, Climate, Weather, Natural Resources)',
                'Human Geography (Population, Culture, Urban Planning, Migration)',
                'Countries and Capitals (World Nations, Political Boundaries, Regions)',
                'Natural Features (Mountains, Rivers, Oceans, Deserts, Forests)',
                'Geographic Phenomena (Volcanoes, Earthquakes, Weather Patterns, Ecosystems)'
            ];
            ...
            const prompt = `Generate a daily geographical learning topic from this category: ${randomCategory}
...
Please provide a response in this exact JSON format:
{
  ...
  "title": "Topic Title",
  "description": "A brief but engaging description of the geographical topic that would interest someone learning about it",
  "difficulty": "${difficultyRank}",
  "keyPoints": [
    "Key geographical point 1 - essential geographical concept to understand",
    "Key geographical point 2 - important geographical fact or feature",
    "Key geographical point 3 - core geographical idea or significance",
    "Key geographical point 4 - geographical context or impact",
    "Key geographical point 5 - connection to broader geographical context"
  ]
}

CRITICAL REQUIREMENTS:
- difficulty MUST be exactly "${difficultyRank}"
- keyPoints MUST contain exactly 5 focused geographical learning objectives
- Each key point should be specific and geographically accurate
- Make the topic interesting and educational about geography
- Keep the description concise but informative
- Focus on essential geographical knowledge, not overwhelming details`;
 ...
 Make questions that test understanding of the topic while being engaging and educational.`;
    ...
    async selectAnswer(answer) {
        ...
        try {
            ...
            this.researchData.partialIndex = this.currentQuestionIndex;
            if (this.userManager && this.userManager.hasUserId()) {
                await this.userManager.updateUserData({ geographyResearchData: this.researchData });
                await this.userManager.forceSaveUserData();
            } else {
                localStorage.setItem("geographyResearchData", JSON.stringify(this.researchData));
            }

...
class ResearchTrainingManager {
    ...
    async loadUserData() {
        try {
            if (this.userManager && this.userManager.hasUserId()) {
                ...
                this.researchData = userData?.politicsResearchData || this.getDefaultResearchData();
    ...
    loadFromLocalStorage() {
        ...
        this.researchData = JSON.parse(localStorage.getItem("politicsResearchData")) || this.getDefaultResearchData();
        return savedData;
    }
        ...
        try {
            ...
            if (this.userManager && this.userManager.hasUserId()) {
                ...
                await this.userManager.updateUserData({
                    politicsResearchData: this.researchData
                });
                ...
                localStorage.setItem("politicsResearchData", JSON.stringify(this.researchData));
        ...
        window.addEventListener('beforeunload', () => {
            ...
            if (this.isQuizActive && !this.researchData.quizResults) {
                try {
                    ...
                    if (this.userManager && this.userManager.hasUserId()) {
                        this.userManager.updateUserData({ politicsResearchData: this.researchData });
    ...
    async loadDailyTopic() {
        ...
        try {
            ...
            const categories = [
                'Political Systems (Democracy, Authoritarianism, Federalism, Parliamentary)',
                'International Relations (Diplomacy, Alliances, Conflicts, Organizations)',
                'Political Theory (Liberalism, Conservatism, Socialism, Political Philosophy)',
                'Government Institutions (Executive, Legislative, Judicial, Bureaucracy)',
                'Political Processes (Elections, Voting, Campaigns, Public Policy)'
            ];
            ...
            const prompt = `Generate a daily political learning topic from this category: ${randomCategory}
...
Please provide a response in this exact JSON format:
{
  ...
  "title": "Topic Title",
  "description": "A brief but engaging description of the political topic that would interest someone learning about it",
  "difficulty": "${difficultyRank}",
  "keyPoints": [
    "Key political point 1 - essential political concept to understand",
    "Key political point 2 - important political fact or principle",
    "Key political point 3 - core political idea or theory",
    "Key political point 4 - political context or practical application",
    "Key political point 5 - connection to broader political context"
  ]
}

CRITICAL REQUIREMENTS:
- difficulty MUST be exactly "${difficultyRank}"
- keyPoints MUST contain exactly 5 focused political learning objectives
- Each key point should be specific and politically accurate
- Make the topic interesting and educational about politics
- Keep the description concise but informative
- Focus on essential political knowledge, not overwhelming details`;
 ...
 Make questions that test understanding of the topic while being engaging and educational.`;
    ...
    async selectAnswer(answer) {
        ...
        try {
            ...
            this.researchData.partialIndex = this.currentQuestionIndex;
            if (this.userManager && this.userManager.hasUserId()) {
                await this.userManager.updateUserData({ politicsResearchData: this.researchData });
                await this.userManager.forceSaveUserData();
            } else {
                localStorage.setItem("politicsResearchData", JSON.stringify(this.researchData));
            }

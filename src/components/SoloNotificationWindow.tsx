...
export const SoloNotificationWindow = ({ onSelectDailyQuest, onClose }: Props) => {
  ...
  return (
    <div className="relative max-w-[560px] w-full mx-auto bg-[#0a1b2e]/90 border-2 border-white/50 rounded-[4px] p-6 sm:p-9 text-white shadow-[0_0_30px_rgba(0,0,0,0.85),inset_0_0_24px_rgba(0,212,255,0.08)] backdrop-blur-md anime-dropdown font-mono">
      ...
      <div className="relative flex items-center justify-center pb-2 mb-5">
      ...
      {/* Notifications List */}
      <div className="border border-white/45 bg-[#061424]/75 p-4 sm:p-5 shadow-[inset_0_0_14px_rgba(0,212,255,0.1)] rounded-[2px] space-y-3">
        {notifications.map((item) => (
          ...
          >
            <div className="flex items-center gap-2.5">
              <span className="text-[#9fd3ff] font-bold text-base">•</span>
              <span className="text-white group-hover:text-[#9fd3ff] transition-colors text-xs sm:text-sm font-medium">
                {item.text}
              </span>
            </div>
            {/* Hollow square symbol from screenshot */}
            <div className="w-3.5 h-3.5 border border-white/60 group-hover:border-[#9fd3ff] group-hover:bg-white/20 shrink-0 transition-colors" />
          </div>
        ))}
      </div>
      ...
      {onClose && (
        <div className="text-center mt-6 pt-4 border-t border-white/20">
          <button
            onClick={onClose}
            className="px-6 py-1.5 text-xs font-mono border border-white/60 text-white hover:bg-white/15 transition-all shadow-[0_0_10px_rgba(0,0,0,0.6)]"
          >
            DISMISS
          </button>
        </div>

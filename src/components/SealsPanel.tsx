...
const RANK_COLOR: Record<SealRank, string> = {
  E: 'text-gray-300 border-gray-400/50',
  D: 'text-emerald-300 border-emerald-400/50',
  C: 'text-cyan-300 border-cyan-400/50',
  B: 'text-blue-300 border-blue-400/50',
  A: 'text-purple-300 border-purple-400/50',
...
export default function SealsPanel() {
  ...
  return (
    <div className="space-y-4">
      ...
      {seals.length === 0 && !creating && (
        <div className="border border-white/30 bg-[#061424]/80 rounded-[2px] p-6 text-center space-y-3 shadow-[inset_0_0_14px_rgba(0,212,255,0.06)]">
          ...
          <p className="text-xs text-white/60 leading-relaxed">
            Weaknesses aren't defeated by ignoring them. Forge one below to start suppressing it.
          </p>
        </div>
      )}
      ...
      <div className="space-y-3">
        {seals.map((seal) => {
          ...
          return (
            <div
              key={seal.id}
              className="border border-white/30 bg-[#061424]/85 rounded-[2px] p-3.5 sm:p-4 space-y-3 shadow-[inset_0_0_14px_rgba(0,212,255,0.05)]"
            >
              {/* Header row */}
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  <ShieldAlert className="w-4 h-4 text-cyan-400 shrink-0" />
                  <span className="font-bold text-sm sm:text-base text-white truncate">{seal.name}</span>
                </div>
                <button
                  type="button"
                  onClick={() => handleDelete(seal)}
                  className="w-7 h-7 shrink-0 flex items-center justify-center text-white/40 hover:text-rose-400 transition-colors"
                  title="Remove Seal"
                >
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                <span
                  className="text-[10px] font-semibold border border-white/25 px-2 py-0.5 rounded-[2px] bg-black/40 text-white/60"
                  title="THEIA's one-time read on how entrenched this weakness is — tunes recovery speed and slip cost"
                >
                  THREAT {seal.threatRank}
                </span>
                <span className={`text-[10px] font-bold border px-2 py-0.5 rounded-[2px] bg-black/40 ${RANK_COLOR[seal.rank]}`}>
                  {seal.rank}-RANK
                </span>
              </div>
              ...
              <div className="text-xs text-white/60">
                Trigger: <span className="text-white/85 font-medium">{seal.cue}</span>
              </div>

              {/* Integrity bar */}
              <div className="space-y-1.5">
                <div className="flex items-center justify-between text-[11px] text-white/60 font-mono font-semibold tracking-wide">
                  <span>INTEGRITY</span>
                  <span className="text-white/90">{seal.integrity}%</span>
                </div>
              ...
              <div className="flex items-center justify-between text-xs text-white/60 border-t border-white/10 pt-2.5">
              ...
              {editingPlanId === seal.id ? (
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    className="w-7 h-7 shrink-0 flex items-center justify-center text-emerald-400 hover:text-emerald-300"
                  >
                  ...
                  <button
                    ...
                    className="w-7 h-7 shrink-0 flex items-center justify-center text-white/50 hover:text-white"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
                ...
                <button
                  ...
                  className="w-full text-left text-xs text-white/70 hover:text-white flex items-start gap-2 border border-white/15 hover:border-white/30 bg-black/20 rounded-[2px] px-3 py-2 transition-colors leading-relaxed"
                >
                  <Pencil className="w-3.5 h-3.5 mt-0.5 shrink-0 text-white/40" />
                  {seal.ifThenPlan ? (
                    <span>
                    ...
                    <span className="italic text-white/50">Add an if-then plan for when the trigger hits...</span>
                  )}
                </button>
              )}

              {/* Wards — environment/friction changes against the cue itself, collapsible */}
              <div className="border border-white/15 rounded-[2px] overflow-hidden bg-black/20">
                <button
                  type="button"
                  ...
                  <div className="p-3 pt-0 space-y-2">
                    ...
                    {seal.wards.map((ward) => (
                      <div key={ward.id} className="flex items-center gap-1.5">
                        <button
                          ...
                          className={`flex-1 min-w-0 text-left text-xs px-2.5 py-1.5 rounded-[2px] border transition-colors truncate ${
                            ward.active
                              ? 'border-emerald-500/50 bg-emerald-950/25 text-emerald-200'
                              : 'border-white/15 bg-black/20 text-white/40 line-through'
                          }`}
                        >
                          {ward.label}
                        </button>
                        <button
                          ...
                          className="w-7 h-7 shrink-0 flex items-center justify-center text-white/30 hover:text-rose-400"
                        >
                    ...
                    <div className="flex items-center gap-1.5">
                      <input
                        ...
                        className="flex-1 min-w-0 bg-black/60 border border-white/20 rounded-[2px] px-2.5 py-1.5 text-xs text-white placeholder:text-white/30 focus:border-emerald-400 focus:outline-none"
                      />
                      <button
                        type="button"
              ...
              {recentEvents.length > 0 && (
                <div className="space-y-1 pt-1 border-t border-white/10">
                  {recentEvents.map((ev) => (
                    <p key={ev.id} className="text-[11px] text-white/45 font-mono leading-relaxed">
                      {ev.message}
                    </p>
                  ))}
                </div>
              ...
              {seal.activeUrgeStartedAt ? (
                (() => {
                  ...
                  return (
                    <div className="border border-cyan-400/40 bg-cyan-950/25 rounded-[2px] p-3 space-y-2.5 text-center">
                      <div className="flex items-center justify-center gap-1.5 text-cyan-300 font-mono">
                        <Clock className={`w-4 h-4 ${timerDone ? '' : 'animate-pulse'}`} />
                        <span className="text-xl font-bold">
                          {timerDone ? "TIME'S UP" : `${mm}:${ss < 10 ? `0${ss}` : ss}`}
                        </span>
                      </div>
                      {seal.ifThenPlan && (
                        <p className="text-xs text-white/70 italic leading-relaxed">"{seal.ifThenPlan}"</p>
                      )}
                      {timerDone ? (
                        <div className="flex items-center gap-1.5">
                          <button
                            ...
                            className="flex-1 py-2 border border-emerald-500/50 bg-emerald-950/40 hover:bg-emerald-900/50 text-emerald-300 rounded-[2px] text-xs font-bold tracking-wider"
                          >
                            IT PASSED
                          </button>
                          <button
                            ...
                            className="flex-1 py-2 border border-rose-500/40 bg-rose-950/25 hover:bg-rose-950/40 text-rose-300 rounded-[2px] text-xs font-bold tracking-wider"
                          >
                            I SLIPPED
                          </button>
                        </div>
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    ...
                    className="flex-1 py-2 border border-cyan-500/50 bg-cyan-950/25 hover:bg-cyan-950/45 text-cyan-300 rounded-[2px] text-xs font-bold tracking-wider flex items-center justify-center gap-1.5 transition-colors"
                  ...
                  <button
                    ...
                    className="flex-1 py-2 border border-rose-500/40 bg-rose-950/25 hover:bg-rose-950/45 text-rose-300 rounded-[2px] text-xs font-bold tracking-wider flex items-center justify-center gap-1.5 transition-colors"
                  >
      ...
      {creating ? (
        <div className="border border-cyan-400/40 bg-cyan-950/10 rounded-[2px] p-3.5 sm:p-4 space-y-3">
          <div className="space-y-2">
            <label className="text-[11px] font-semibold text-white/60 tracking-wide">WEAKNESS</label>
            <input
              type="text"
              value={newName}
              onChange={(e) => {
              ...
              placeholder="e.g. Doomscrolling after midnight"
              className="w-full bg-black/60 border border-white/30 rounded-[2px] px-3 py-2 text-sm text-white placeholder:text-white/30 focus:border-cyan-400 focus:outline-none"
            />
          </div>
          <div className="space-y-2">
            <label className="text-[11px] font-semibold text-white/60 tracking-wide">TRIGGER (CUE)</label>
            <input
              type="text"
              value={newCue}
              onChange={(e) => {
          ...
          <div className="space-y-2">
            <label className="text-[11px] font-semibold text-white/60 tracking-wide">IF-THEN PLAN (OPTIONAL)</label>
            <input
              type="text"
              value={newPlan}
              onChange={(e) => {
              ...
              placeholder="When [trigger], I will [replacement]..."
              className="w-full bg-black/60 border border-white/30 rounded-[2px] px-3 py-2 text-sm text-white placeholder:text-white/30 focus:border-cyan-400 focus:outline-none"
            />
          </div>
          ...
          <div className="flex items-center gap-1.5 pt-1">
            <button
              ...
              className="py-2.5 px-3 border border-emerald-500/50 bg-emerald-950/50 hover:bg-emerald-900/60 text-emerald-300 font-mono text-xs font-bold tracking-wider rounded-[2px] transition-all flex items-center justify-center gap-1.5 disabled:opacity-40"
            ...
            <button
              ...
              disabled={!canAssess || assessing}
              className="flex-1 py-2.5 px-3 border-2 border-cyan-400/60 bg-cyan-950/50 hover:bg-cyan-900/60 text-cyan-300 font-mono text-xs font-bold tracking-widest rounded-[2px] transition-all flex items-center justify-center gap-1.5 disabled:opacity-40"
            >
              <Sparkles className={`w-4 h-4 ${assessing ? 'animate-spin' : ''}`} />
              <span>{assessing ? 'ANALYZING...' : 'REQUEST THEIA ASSESSMENT'}</span>
            </button>
          </div>

          {assessment && (
            <div className="border border-cyan-400/40 bg-[#07172b]/80 rounded-[2px] p-3 space-y-2.5">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-bold tracking-[0.15em] text-cyan-300">THEIA ASSESSMENT</span>
                <span className="text-[10px] text-white/50">
                  {assessment.origin === 'system' ? 'PRECISION ENGINE' : 'AI ADAPTED'}
                </span>
              </div>
              <p className="text-xs text-white/90 leading-relaxed italic">"{assessment.rationale}"</p>
              <div className="flex items-center gap-1.5 text-xs">
                <span className="text-white/60">Threat Rank:</span>
                <span className="font-bold text-amber-300">{assessment.threatRank}</span>
              </div>

              {!assessment.cueSpecificEnough && assessment.cueFeedback && (
                <div className="border border-amber-500/50 bg-amber-950/30 rounded-[2px] p-2.5 space-y-1">
                  <div className="flex items-center gap-1.5 text-amber-300 text-xs font-bold">
                    <AlertTriangle className="w-3.5 h-3.5" />
                    CUE TOO VAGUE
                  </div>
                  <p className="text-xs text-white/80 leading-relaxed">{assessment.cueFeedback}</p>
                </div>
              )}
              ...
              <button
                ...
                className="w-full text-left border border-cyan-400/40 bg-cyan-950/25 hover:bg-cyan-950/45 rounded-[2px] p-2.5 space-y-1 transition-colors"
              >
                <div className="flex items-center gap-1.5 text-[10px] font-semibold text-cyan-300/90 tracking-wider">
                  <Sparkles className="w-3.5 h-3.5" />
                  THEIA-CRAFTED PLAN — TAP TO USE
                </div>
                <p className="text-xs text-white leading-relaxed">{assessment.suggestedPlan}</p>
              </button>
            </div>
          )}
          ...
          <div className="flex items-center gap-2 pt-1">
            <button
              ...
              disabled={!assessment}
              className="flex-1 py-2 border-2 border-cyan-400 bg-cyan-950/80 hover:bg-cyan-900 text-cyan-300 hover:text-white rounded-[2px] text-xs font-bold tracking-wider disabled:opacity-40 disabled:cursor-not-allowed"
            >
              FORGE SEAL
            </button>
            <button
        ...
        <button
          ...
          className="w-full py-2.5 border border-cyan-500/50 bg-cyan-950/25 hover:bg-cyan-950/45 text-cyan-300 rounded-[2px] text-xs font-bold tracking-wider flex items-center justify-center gap-1.5"
        >
          <Plus className="w-4 h-4" />
          FORGE NEW SEAL
        </button>
